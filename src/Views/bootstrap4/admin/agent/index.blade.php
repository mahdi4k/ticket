...
{!! link_to_route(
    ...
    trans('ticketit::admin.btn-create-new-agent',[],'fa'), null,
...
@section('ticketit_content')
    @if ($agents->isEmpty())
        <h3 class="text-center">{{ trans('ticketit::admin.agent-index-no-agents',[],'fa') }}
            {!! link_to_route($setting->grab('admin_route').'.agent.create', trans('ticketit::admin.agent-index-create-new',[],'fa')) !!}
        </h3>
    @else
        <div id="message"></div>
        <table class="table table-hover mb-0">
            <thead>
                <tr>
                    <th>{{ trans('ticketit::admin.table-id',[],'fa') }}</th>
                    <th>{{ trans('ticketit::admin.table-name',[],'fa') }}</th>
                    <th>{{ trans('ticketit::admin.table-categories',[],'fa') }}</th>
                    <th>{{ trans('ticketit::admin.table-join-category',[],'fa') }}</th>
                    <th>{{ trans('ticketit::admin.table-remove-agent',[],'fa') }}</th>
                </tr>
            </thead>
            <tbody>
