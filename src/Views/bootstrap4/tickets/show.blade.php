...
<div>
    ...
    @if($u->isAdmin())
        @if($setting->grab('delete_modal_type') == 'builtin')
            {!! link_to_route(
                            $setting->grab('main_route').'.destroy', trans('ticketit::lang.btn-delete',[],'fa'), $ticket->id,
                            [
...
{{-- // OR; Modal Window: 1/2 --}}
            ...
            <button type="button"
                    ...
                    data-title="{!! trans('ticketit::lang.show-ticket-modal-delete-title', ['id' => $ticket->id],'fa') !!}"
                    data-message="{!! trans('ticketit::lang.show-ticket-modal-delete-message', ['subject' => $ticket->subject],'fa') !!}"
             >
...
@section('ticketit_extra_content')
    <h2 class="mt-5 text-right">{{ trans('ticketit::lang.comments',[],'fa') }}</h2>
    ...
    <script>
        $(document).ready(function() {
            $( ".deleteit" ).click(function( event ) {
                ...
                if (confirm("{!! trans('ticketit::lang.show-ticket-js-delete',[],'fa') !!}" + $(this).attr("node") + " ?"))
                {
